...
def load_from_csv(main_window):
    ...
    try:
        # First try semicolon-delimited (export format), then fall back to comma-delimited
        ...
        # Branch A: matrix format with time ranges and names
        if first_col == "godzina":
        ...
        # Branch B: employee rows with D/N/0/O codes
        elif first_col == "employee":
            date_list = [d.strip() for d in header[1:] if d.strip()]
            # Validate date format (YYYY-MM-DD)
            try:
                parsed_dates = [datetime.strptime(d, "%Y-%m-%d").date() for d in date_list]
            except Exception:
                QMessageBox.warning(main_window, "Błąd", "Daty w nagłówku muszą być w formacie YYYY-MM-DD.")
                return
            ...
            for row in rows[1:]:
                ...
                for i, val in enumerate(row[1:]):
                    ...
                    d = parsed_dates[i]
                    if code == "D":
                        sdt = datetime(d.year, d.month, d.day, DAY_START, 0)
                        edt = datetime(d.year, d.month, d.day, DAY_END, 0)
                        schedule_entries.append({
                            "Shift Start": sdt,
                            "Shift End": edt,
                        ...
                        sdt = datetime(d.year, d.month, d.day, NIGHT_START, 0)
                        # Night ends next day at 07:00
                        from datetime import timedelta
                        next_day = d + timedelta(days=1)
                        edt = datetime(next_day.year, next_day.month, next_day.day, 7, 0)
                        schedule_entries.append({
                            "Shift Start": sdt,
                            "Shift End": edt,
                            "Assigned To": employee
            ...
            # Keep poll_dates in given order
            main_window.poll_dates = [d.strftime("%Y-%m-%d") for d in parsed_dates]

            # Define day ranges 07:00 -> next-day 07:00 for each date
            from datetime import timedelta
            day_ranges = {}
            for d in parsed_dates:
                start_dt = datetime(d.year, d.month, d.day, 7, 0)
                nd = d + timedelta(days=1)
                end_dt = datetime(nd.year, nd.month, nd.day, 7, 0)
                day_ranges[d.strftime("%Y-%m-%d")] = (start_dt, end_dt)
            main_window.day_ranges = day_ranges

            # Build slots and participants
            full_slots = sorted(
                [(e["Shift Start"], e["Shift End"]) for e in schedule_entries],
                key=lambda x: x[0]
            )
            main_window.full_slots = full_slots
            ...
            if hasattr(main_window, "schedule_widget"):
                # Use 60-min visualization slots by default
                shift_duration = 60
                main_window.schedule_widget.load_schedule_matrix(
                    schedule_data=schedule_entries,
                    participants=main_window.participants,
            ...
            QMessageBox.information(main_window, "Import CSV",
                                    "Niepoprawny format CSV.\nOczekiwano nagłówka 'Godzina' lub 'Employee'.")
            return
