...
class CollapsibleSidebar(QFrame):
    ...
    def __init__(self, parent=None, initial_mode=False):
        ...
        # In initial mode, show a CSV button under Timeful to create schedules from CSV
        if initial_mode:
            csv_icon = QIcon(get_icon_path("load_csv"))
            self.btn_timeful_csv = SidebarButton(icon=csv_icon, text="CSV grafiku", checkable=False)
            self.btn_timeful_csv.clicked.connect(lambda: self.sig_timeful_csv.emit())
            self.main_layout.addWidget(self.btn_timeful_csv)
    ...
    def toggle_sidebar(self):
        ...
        for attr_name in [
            ...
            "btn_doc", "btn_go_initial", "btn_timeful_csv"
        ]:
    ...
    def update_icons(self, initial_mode=False):
        ...
        if current_theme in ["light"]:
            if not initial_mode:
                ...
                if hasattr(self, "btn_timeful_csv"):
                    self.btn_timeful_csv.setIcon(QIcon(get_icon_path("load_csv", variant="light")))
            ...
            if not initial_mode:
                ...
                if hasattr(self, "btn_timeful_csv"):
                    self.btn_timeful_csv.setIcon(QIcon(get_icon_path("load_csv", variant="dark")))
            ...
            if not initial_mode:
                ...
                if hasattr(self, "btn_timeful_csv"):
                    self.btn_timeful_csv.setIcon(QIcon(get_icon_path("load_csv")))
