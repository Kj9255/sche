...
class InitialSetupDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.sidebar.sig_timeful_csv.connect(self.on_timeful_csv)
    ...
    def on_timeful_csv(self):
        """
        Allow selecting a CSV to create schedule directly (Employee/D,N,0 format).
        """
        from PyQt6.QtWidgets import QFileDialog
        from core.export_handlers import load_from_csv

        # Reuse the main CSV loader by temporarily creating a dummy main window-like object
        # Instead of complex plumbing, we will open the file, then pass control back to main.
        try:
            filepath, _ = QFileDialog.getOpenFileName(self, "Wczytaj CSV", "", "CSV Files (*.csv)")
            if not filepath:
                return
            # We'll store the file path into settings so that main can detect it
            self.settings.setValue("startup_csv_path", filepath)
            # Mark engine as Timeful for UI parity
            self.loaded_engine = "Timeful"
            # Finish the dialog – main will load CSV after creating the window
            self.accept()
        except Exception as e:
            self.show_error(str(e))
